<template>
	<div class="poeui-page">
		<header class="poeui-content-header flex-row flex-line-center" >
			<h2 class="poeui-content-title">热门排行</h2>
			<div class="flex-item"></div>
		</header>
		<tabs :items="[{value:1,label:'视频'},{value:2,label:'音乐'},{value:3,label:'主播'},{value:4,label:'话题'}]" v-model="currType"/>
		<article class="poeui-content-box" v-if="currType==1">
			<block class="file-block">
				<div slot="aside" class="hor-right">
					<a class="btn-block success" style="width:1rem;display: inline-block;line-height: .22rem;text-align: center;" href="/pin/ranking/add?t=1">添加</a>
				</div>
				<section >
					<data-table :columns="columns.video"  :setting="false" v-if="ready.video && list.videos.length>0">
						<template v-for="(v,index) in list.videos">
							<tr>
					...
					<div class="mv-50 hor-center tip" v-else-if="ready.video && list.videos.length<=0"> == 没有数据 == </div>
...
<script>
	...
	export default {
		...
		route:{
			path:['/pin/ranking','/pin/ranking/list'],
			meta:{
				app:'pin',
				catch:true,
				menu:'ranking',
		...
		data() {
			return {
				...
				columns:{
					video:['指数',{title:'视频内容',align:'left'},'播主','点赞数','评论数','操作'],
					music:[{title:'音乐'	,align:'left'},'时长','总使用人数', '操作'],
					anchor:['排行', {title:'播主',align:'left'}, '飞瓜指数', '粉丝数', '点赞', '评论', '转发', '操作'],
					topic:[{title:'话题',align:'left'}, '参与', '操作'],
				},
				list:{
					videos:[],
					musics:[],
		...
		watch:{
			currType(v){
				this.$router.push({path:'/pin/ranking?t='+v})
			},
		},
		mounted(){
		...
		beforeRouteUpdate (to, from, next) {
			...
			switch(parseInt(to.query.t)){
				...
				break
				default:
					if ( !this.ready.video ) this.loadVideos()
				break
			}
		...
		methods:{
			loadVideos(page){
				this.ready.video = false
				http.request(urls.data,{kind:1,rows:50,page:page||1},rlt=>{
					if (rlt.status==1) {
						this.list.videos=rlt.data||[]
					}
					else{
						$$.ui.modal(rlt.msg,{cancel:false,align:'center',mask:true})
					}
					this.ready.video = true
				})
			},
			loadMusics(page){
				this.ready.music = false
				http.request(urls.data,{kind:2,rows:50,page:page||1},rlt=>{
					if (rlt.status==1) {
						this.list.musics=rlt.data||[]
					}
					else{
						$$.ui.modal(rlt.msg,{cancel:false,align:'center',mask:true})
					}
			...
			loadAnchors(page){
				this.ready.anchor = false
				http.request(urls.data,{kind:3,rows:50,page:page||1},rlt=>{
					if (rlt.status==1) {
						this.list.anchors=rlt.data||[]
					}
					else{
						$$.ui.modal(rlt.msg,{cancel:false,align:'center',mask:true})
					}
					this.ready.anchor = true
				})
			},
			loadTopic(page){
				this.ready.topic = false
				http.request(urls.data,{kind:4,rows:50,page:page||1},rlt=>{
					if (rlt.status==1) {
						this.list.topics=rlt.data||[]
					}
					else{
						$$.ui.modal(rlt.msg,{cancel:false,align:'center',mask:true})
					}
					this.ready.topic = true
				})
			},
			addBrand(){
			...
			handleCurrentChange(row){
				if (this.editor.good) {
					this.list.videos.splice(this.current_index,1,row)
			...
			handleGoodStatusToggle(index,status){
				$$.ui.loading('',{ref:'l1'})
				try{
	                http.request(urls.good.save,{id:this.list.videos[index].id,status:status},(rlt)=>{
	                	$$.ui.hide({ref:'l1'})
	                    if (rlt.status!=1) {
	                    	$$.ui.modal(rlt.msg,{
	                    		cancel:false,
                ...
                $$.ui.modal('确实要删除当前产品吗？',{
                	...
                	confirm:()=>{
	                    ...
	                    http.request(urls.good.delete,{ids:[row.id]},(rlt)=>{
	                    	$$.ui.hide({ref:'l1'})
	                        if (rlt.status==1) {
	                        	this.list.videos.splice(index,1)
